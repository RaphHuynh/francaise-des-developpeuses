...
const { VITE_BASE_URL: baseUrl, GIT_CLIENT_ID, GIT_SCOPE } = import.meta.env;
    ...
    const githubLogin = () => {
        const response = fetch(url, {
            ...
            if (response.type == "opaqueredirect") {
                ...
                window.location.replace(`https://github.com/login/oauth/authorize?redirect_uri=${encodeURIComponent(`${baseUrl}/github/callback`)}&response_type=code&client_id=${GIT_CLIENT_ID}&scope=${GIT_SCOPE}`
                ); // create one and only one request
            }
        })
            .catch(function (err) {
            ...
            <section className="flex flex-col-reverse xl:flex-row items-center justify-center w-full pt-10 h-screen gap-4">
                ...
                <article className="flex flex-col h-screen xl:w-3/6 w-full bg-black pt-14">
                    ...
                    <button onClick={githubLogin} className="flex items-center gap-2 text-4xl uppercase text-beige my-10 mx-5 md:mx-16 justify-center border-2 border-[#F7F4E9] py-2 rounded-full hover:bg-[#F7F4E9] hover:text-black transition delay-100">
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" className="bi bi-github" viewBox="0 0 16 16">
