...
function Card(){
    ...
    return (
        <article className=": sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {members.map((member) =>(
                <Link to={`${member.id_member}`} key={member.id_member} className="transition delay-75  md:border-b h-min hover:scale-105 duration-50 md:border">
                    <img src={"http://127.0.0.1:8000/member/image_portfolio_by_id?id_member="+member.id_member} alt={`${member.username}`} className="object-cover w-full h-56 hover:contrast-125 duration-200 md:border-b-2"></img>
